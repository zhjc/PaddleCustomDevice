...
def generate_linear_npu_op():
    def pattern(x, y, z):
        return paddle.add(paddle.matmul(x, y), z)

    def replace(x, y, z):
        return paddle.incubate.passes.ir.PassDesc.OP.linear_npu_op(Input=x, Weight=y, Bias=z)

    return pattern, replace

@paddle.jit.to_static(input_spec=[paddle.static.InputSpec([1, 3, 32, 24], 'float32', 'input'),  paddle.static.InputSpec([1, 3, 24, 48], 'float32', 'weight'),  paddle.static.InputSpec([48], 'float32', 'bias')])
def func(x, y, z):
    return paddle.add(paddle.matmul(x, y), z)
...
pass_builder.append_pass('generate_linear_npu_op')
...
input_data = np.random.randn(1, 3, 32, 24).astype('float32')
input_tensor.copy_from_cpu(input_data)
weight_tensor = predictor.get_input_handle('weight')
weight_data = np.random.randn(1, 3, 24, 48).astype('float32')
weight_tensor.copy_from_cpu(weight_data)
bias_tensor = predictor.get_input_handle('bias')
bias_data = np.random.randn(48).astype('float32')
...
print(results)
print(paddle.abs(paddle.to_tensor(input_data)))
