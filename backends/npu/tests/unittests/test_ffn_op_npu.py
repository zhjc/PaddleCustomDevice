...
def generate_ffn():
    ...
    def replace(x, y, z):
        return paddle.incubate.passes.ir.PassDesc.OP.ffn(Input=x, Weight=y, Bias=z)
...
input_data = np.random.randn(3, 9, 64).astype('float32')
input_tensor.copy_from_cpu(input_data)
weight_tensor = predictor.get_input_handle('weight')
weight_data = np.random.randn(64, 16).astype('float32')
weight_tensor.copy_from_cpu(weight_data)
bias_tensor = predictor.get_input_handle('bias')
bias_data = np.random.randn(16).astype('float32')
...
print(results)
print([paddle.nn.functional.gelu(paddle.add(paddle.matmul(paddle.to_tensor(input_data), paddle.to_tensor(weight_data)), paddle.to_tensor(bias_data))).numpy()])
